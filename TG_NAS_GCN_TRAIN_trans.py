...
import logging
import argparse
from GNN_proxy_tool.smaller_models import GCN
...
OPS = ["input", "none", "skip_connect", "conv1x1-bn-relu", "conv3x3-bn-relu", "output"]
...
def validate(model, loss, validation_loader, logging=None):
    ...
    with torch.no_grad():
        for i_batch, sample_batched in tqdm(enumerate(validation_loader), total=len(validation_loader), desc="Validation"):
            # print(f"\nBatch {i_batch}")
            # print("Adjacency matrices shape:", sample_batched['adjacency_matrix'].shape)
            # print("Operations shape:", sample_batched['operations'].shape)
            # print("Accuracy:", sample_batched['accuracy'])
            # Optionally, print the actual arrays if they're small.
            # print("Adjacency matrix sample:\n", sample_batched['adjacency_matrix'][0])
            # print("Operations sample:\n", sample_batched['operations'][0])
            # if i_batch >= 2:  # just print 3 batches
            #     break
            adjs, features, accuracys = sample_batched['adjacency_matrix'], sample_batched['operations'], \
                                        sample_batched['accuracy'].view(-1, 1)
            adjs, features, accuracys = adjs.cuda(), features.cuda(), accuracys.cuda()
        ...
        # print("Unique ground truth values:", np.unique(ground_truth))
...
def fit(args, lr, num_epoch, selected_loss, ifsigmoid, batch_size, logger_complete, save_path):
    ...
    if args.use_201_to_test:
        ...
        validation_loader = DataLoader(dataset_201, batch_size=batch_size,
                                                shuffle=True)
    if args.use_trans_to_test:
        dataset_301 = Nas_201_Dataset(pickle_file=args.data_path_trans)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='/home/jingchl6/.local/TG-NAS/new_data/nasbench_101_dataset_sentance_transformer_fine_tuned_sentence_transformer_long_embedding.pkl', help='location of the data')
    ...
    parser.add_argument('--data_path_trans', type=str, default='/home/jingchl6/.local/TG-NAS/new_data/transnas/transnas_micro_autoencoder_test_ssim.pkl', help='location of the 301 data')
    ...
    parser.add_argument('--use_trans_to_test', action=argparse.BooleanOptionalAction)
