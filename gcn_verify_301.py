...
OPS_301 = ['input', 'none', 'skip_connect', 'sep_conv_3x3', 'sep_conv_5x5',
       'dil_conv_3x3', 'dil_conv_5x5', 'avg_pool_3x3', 'max_pool_3x3', 'output']
def validate(args, model):

    loss = torch.nn.MSELoss()
    if args.test_301:
        dataset = Nas_301_Dataset(pickle_file=args.data_path_301)
    else:    
    ...
    with torch.no_grad():
        ...
        np.save('301_file'+'_'+args.comment, combined)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='/home/jingchl6/.local/TG-NAS/new_gnn_result/GNN_Evaluation_Result_gchl6/.local/TG-NAS/new_data/nasbench_101_dataset_sentance_transformer_fine_tuned_sentence_transformer_long_embedding_384_use301_test/checkpoint128_0.001100_4layer_no_weightdecay/checkpoint.pth.tar', help='location of the GNN model')
    parser.add_argument('--batch_size', type=int, default=128, help='batch size')
    parser.add_argument('--embedding_size', type=int, default=384, help='embedding size')
    parser.add_argument('--comment', type=str, default='', help='comment')
    parser.add_argument('--test_301', action=argparse.BooleanOptionalAction)
