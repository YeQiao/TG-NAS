...
def prune_func_rank(xargs, arch_parameters, model_config, model_config_thin, loader, lrc_model, search_space, precision=10, prune_number=1, xshape = (1, 3, 32, 32)):
    ...
    rankings = {}  # dict of (cell_idx, edge_idx, op_idx): [ntk_rank, regions_rank, latency]
    ...
    for idx, data in enumerate(latency_all):
        if idx == 0:
            ...
            if data[0] == latency_all[idx-1][0]:
                # same latency as previous
                rankings[data[1]].append(rankings[latency_all[idx-1][1]][2])
            else:
                rankings[data[1]].append(rankings[latency_all[idx-1][1]][2]+1)

    
    rankings_list = [[k, v] for k, v in rankings.items()]  # list of (cell_idx, edge_idx, op_idx), [ntk_rank, regions_rank, latency_rank]
    # ascending by sum of two rankings
    # rankings_sum = sorted(rankings_list, key=lambda tup: sum(tup[1]), reverse=False)  # list of (cell_idx, edge_idx, op_idx), [ntk_rank, regions_rank, latency]
    rankings_sum = sorted(rankings_list, key=lambda tup: np.average(tup[1], weights = [(1-xargs.latency_weight)/2, (1-xargs.latency_weight)/2, xargs.latency_weight]), reverse=False)  # list of (cell_idx, edge_idx, op_idx), [ntk_rank, regions_rank, latency_rank]
    
    edge2choice = {}  # (cell_idx, edge_idx): list of (cell_idx, edge_idx, op_idx) of length prune_number
    for (cell_idx, edge_idx, op_idx), [ntk_rank, regions_rank, latency_rank] in rankings_sum:
...
def main(xargs):
    ...
    logger.log('||||||| {:10s} ||||||| {:}'.format("rank weights (ntk, lr, latency)", [(1-xargs.latency_weight)/2, (1-xargs.latency_weight)/2, xargs.latency_weight]))
    ...
    logger.log('||||||| {:10s} ||||||| {:}'.format("rank weights (ntk, lr, latency)", [(1-xargs.latency_weight)/2, (1-xargs.latency_weight)/2, xargs.latency_weightt]))
    logger.close()
